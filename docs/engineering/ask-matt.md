...
It **does no work itself**. It doesn't grill, write a spec, or fix anything — it only orients. It exists for the **user-invoked** skills above all: nothing fires those for you, so *you* have to remember they exist, and `ask-matt` is the memory you offload that to. It also points at the model-invoked skills you'd reach for by name — the build and debug set (`/tdd`, `/diagnosing-bugs`, `/production-debugging`, `/prototype`, `/code-review`, `/resolving-merge-conflicts`), the from-nothing pair (`/system-design`, `/full-stack-mvp`, plus `/ui-components`), the codebase-health group (`/onboarding-audit`, `/clean-architecture`, `/optimizing-performance`), `/four-hats` for high-stakes changes, and the two vocabulary references, `/domain-modeling` and `/codebase-design`. It answers "which one, and when", then hands you off to the skill that actually does the job.
...
The idea `ask-matt` gives you to think with is the **flow** — a path *through* the skills rather than a single one. Most work runs along one **main flow** (idea → ship: grill → spec → tickets → implement → review), a handful of **on-ramps** merge onto it (a triage lane for incoming bugs and requests; something broken; an unfamiliar codebase; a foggy effort too big for one session), a **starting-from-nothing** pair sits off to the side, and everything else is a **standalone** you reach for on its own. Ask a question and you get placed on the right flow, at the right step — not just handed a tool.
