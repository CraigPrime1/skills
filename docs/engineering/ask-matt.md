...
- **On-ramps**, for a situation that generates work and then merges onto the main flow: incoming bug reports, something broken (including a live production failure, which goes to [production-debugging](https://aihero.dev/skills-production-debugging) first), an unfamiliar codebase ([onboarding-audit](https://aihero.dev/skills-onboarding-audit)), or an effort too foggy and too large to hold in one session.
- **Starting from nothing**, for a greenfield build that *is* the work rather than merging onto the main flow: [system-design](https://aihero.dev/skills-system-design) for one backend service, [full-stack-mvp](https://aihero.dev/skills-full-stack-mvp) for a whole application.
- **Standalones**, off every flow, reached for on their own terms: the prototype, the questionnaire, the merge conflict you are already sitting in, a reusable [UI component](https://aihero.dev/skills-ui-components), a high-stakes change run through [four-hats](https://aihero.dev/skills-four-hats), or [agent-safety](https://aihero.dev/skills-agent-safety) when you want Cursor Grok, Claude Code, and Codex fenced from destructive shell.
