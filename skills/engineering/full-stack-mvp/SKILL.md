---
name: full-stack-mvp
description: Build a complete, production-ready application from scratch — architecture, file structure, database schema, API endpoints, UI, and working code. Use when the user wants a new app or product built end to end, a greenfield project, or a startup MVP that has to be real rather than a demo.
---
...
Designing one system rather than building a whole app? Use `/system-design`. This skill borrows its Phases 1–6 whenever the backend needs real scale thinking.
...
If the user hasn't given you enough to write this, ask — or reach for `/grill-with-docs`. Building the wrong loop beautifully is the most expensive outcome available.
...
- The **ownership key** on every row that belongs to a user or tenant — retrofitting multi-tenancy is a rewrite; a `user_id`/`org_id` column from day one is free.
...
Non-negotiable for "production-ready", and cheap now, expensive later:

- [ ] **Authentication**, and **authorisation on every endpoint** — ownership checked server-side, not by hiding UI.
...
- [ ] **Structured logging** with request ids, plus error reporting.
...
- [ ] **Tests at the seams** — the core loop end to end, plus unit tests for the domain rules. Not 100% coverage; the loop must be protected.
- [ ] **CI** running typecheck, lint, tests, build on every push.
- [ ] **Deploy** — one documented command or pipeline, and a health check.
- [ ] **README** — how to run it, how to test it, how to deploy it.

## Phase 7 — Build in vertical slices

Never layer-by-layer. Each slice goes schema → endpoint → UI → test and **runs end to end** before the next begins, so the app is demoable at every commit and integration risk is paid down first. Order the slices so the riskiest or most uncertain part of the loop is slice one.

Drive `/tdd` for the domain rules, and close out with `/code-review` before calling it done.
