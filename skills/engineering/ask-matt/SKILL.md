...
- **`/agent-safety`** installs **guardrails** that block dangerous agent shell commands (`git push`, `git reset --hard`, force-clean, `curl | bash`) before they run. Cursor Grok reads `.cursor/hooks.json` (project hooks also run in the cloud). Claude Code gets a PreToolUse hook. Codex gets the `AGENTS.md` section only. Reach for it when you want every harness fenced, not Claude Code alone.
